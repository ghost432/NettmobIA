...
const parseOutput = (output) => {
    ...
    if ((trimmedOutput.startsWith('{') && trimmedOutput.endsWith('}')) || (trimmedOutput.startsWith('[') && trimmedOutput.endsWith(']'))) {
        try {
            const parsedOutput = json5_1.default.parse(trimmedOutput);
            return parsedOutput;
        }
        catch (e) {
            return output;
...
const executeJavaScriptCode = async (code, sandbox, options = {}) => {
    ...
    if (shouldUseSandbox) {
        try {
            ...
            const sbx = await code_interpreter_1.Sandbox.create({ apiKey: process.env.E2B_APIKEY, timeoutMs: timeout });
            // Install libraries
        ...
        const deps = exports.availableDependencies.concat(externalDeps);
