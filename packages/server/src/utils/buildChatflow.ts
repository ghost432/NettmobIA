...
const getChatHistory = async ({
    ...
    for (const endingNode of endingNodes) {
       const endingNodeData: EndingNodeData = (nodes.find((node: any) => node.data?.name === 'End')?.data || {}) as EndingNodeData;
        if (!endingNodeData.inputs?.[0]?.memory) continue

        const memoryNodeId = endingNodeData.inputs?.[0]?.memory.split('.')[0].replace('{{', '')
        const memoryNode = nodes.find((node) => node.data.id === memoryNodeId)
    ...
    if (isAgentFlow) {
        ...
        if (streamResults) {
            ...
            if (agentflow.followUpPrompts) {
                ...
                if (generatedFollowUpPrompts && 
					typeof generatedFollowUpPrompts === 'object' && 
					generatedFollowUpPrompts !== null &&
					'questions' in generatedFollowUpPrompts) {
        ...
        if (chatflow.followUpPrompts) {
            ...
            if (followUpPrompts && 
			typeof followUpPrompts === 'object' && 
			followUpPrompts !== null &&
			'questions' in followUpPrompts) {
...
const checkIfStreamValid = async (
    ...
    let isStreamValid = false
    for (const endingNode of endingNodes) {
        const endingNodeData: EndingNodeData = (nodes.find((node: any) => node.data?.name === 'End')?.data || {}) as EndingNodeData; // Ensure endingNodeData is never undefined
        ...
        if (endingNodeData?.outputs &&
			...
			throw new Error(
				`Output of ${endingNodeData.label || 'Unknown'} (${endingNodeData.id || 'Unknown'}) must be ${endingNodeData.label || 'Unknown'}, can't be an Output Prediction`
			)
		}
