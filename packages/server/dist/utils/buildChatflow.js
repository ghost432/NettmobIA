...
const getChatHistory = async ({ endingNodes, nodes, chatflowid, appDataSource, componentNodes, incomingInput, chatId, isInternal, isAgentFlow }) => {
    ...
    for (const endingNode of endingNodes) {
        const endingNodeData = (nodes.find((node) => node.data?.name === 'End')?.data || {});
        if (!endingNodeData.inputs?.[0]?.memory)
            continue;
...
const checkIfStreamValid = async (endingNodes, nodes, streaming) => {
    ...
    let isStreamValid = false;
    for (const endingNode of endingNodes) {
        const endingNodeData = (nodes.find((node) => node.data?.name === 'End')?.data || {}); // Ensure endingNodeData is never undefined
        ...
        if (endingNodeData?.outputs &&
            ...
            throw new Error(`Output of ${endingNodeData.label || 'Unknown'} (${endingNodeData.id || 'Unknown'}) must be ${endingNodeData.label || 'Unknown'}, can't be an Output Prediction`);
        }
