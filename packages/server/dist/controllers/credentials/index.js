...
const getAllCredentials = async (req, res, next) => {
    try {
        const apiResponse = await credentials_1.default.getAllCredentials(req.query.credentialName, req.user?.activeWorkspaceId);
        return res.json(apiResponse);
    }
    catch (error) {
        next(error);
...
const getCredentialById = async (req, res, next) => {
    try {
        ...
        const apiResponse = await credentials_1.default.getCredentialById(req.params.id, req.user?.activeWorkspaceId);
        return res.json(apiResponse);
    }
    catch (error) {
        next(error);
