...
class RefactorEnterpriseDatabase1737076223692 {
    ...
    async populateTable(queryRunner) {
        ...
        for (let workspace of workspaces) {
            await queryRunner.query(`update \`workspace\` set \`createdBy\` = '${adminUserId}', \`updatedBy\` = '${adminUserId}' where \`id\` = '${workspace.id}';`);
        }
        /*-------------------------------------
        ----------- workspace_user ------------
        --------------------------------------*/
        ...
        for (let workspaceUser of workspaceUsers) {
            ...
            else if (workspace.name === workspace_entity_1.WorkspaceName.DEFAULT_PERSONAL_WORKSPACE && !user.lastLogin) {
