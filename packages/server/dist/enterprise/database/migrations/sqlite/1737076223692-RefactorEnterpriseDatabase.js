...
exports.RefactorEnterpriseDatabase1737076223692 = void 0;
const _1743758056188_FixOpenSourceAssistantTable_1 = require("../../../../database/migrations/sqlite/1743758056188-FixOpenSourceAssistantTable");
const encryption_util_1 = require("../../../utils/encryption.util");
const login_method_entity_1 = require("../../entities/login-method.entity");
...
const _1729130948686_LinkWorkspaceId_1 = require("./1729130948686-LinkWorkspaceId");
class RefactorEnterpriseDatabase1737076223692 {
    constructor() {
        this.name = 'RefactorEnterpriseDatabase1737076223692';
    }
    async modifyTable(queryRunner) {
        /*-------------------------------------
        --------------- user -----------------
        --------------------------------------*/
        ...
        await queryRunner.query(`
            create table "user" (
                ...
                "status" varchar(20) default '${user_entity_1.UserStatus.UNVERIFIED}' not null,
        ...
        await queryRunner.query(`
            create table "organization" (
                "id" uuid default (lower(substr(hex(randomblob(16)), 1, 8) || '-' || substr(hex(randomblob(16)), 9, 4) || '-' || substr('1' || substr(hex(randomblob(16)), 9, 3), 1, 4) || '-' || substr('8' || substr(hex(randomblob(16)), 13, 3), 1, 4) || '-' || substr(hex(randomblob(16)), 17, 12))) primary key,
                "name" varchar(100) default '${organization_entity_1.OrganizationName.DEFAULT_ORGANIZATION}' not null,
        ...
        await queryRunner.query(`
            create table "login_method" (
                ...
                "status" varchar(20) default '${login_method_entity_1.LoginMethodStatus.ENABLE}'  not null,
                "createdDate" timestamp default current_timestamp not null,
                "updatedDate" timestamp default current_timestamp not null,
                "createdBy" uuid null,
                "updatedBy" uuid null,
        ...
        await queryRunner.query(`
            create table "organization_user" (
                ...
                "status" varchar(20) default '${organization_user_entity_1.OrganizationUserStatus.ACTIVE}' not null,
                "createdDate" timestamp default current_timestamp not null,
                "updatedDate" timestamp default current_timestamp not null,
                "createdBy" uuid not null,
                "updatedBy" uuid not null,
        ...
        await queryRunner.query(`
            create table "workspace_user" (
                ...
                "userId" uuid not null,
                "roleId" uuid not null,
                "status" varchar(20) default '${workspace_user_entity_1.WorkspaceUserStatus.INVITED}' not null,
    ...
    async deleteWorkspaceWithoutUser(queryRunner) {
        ...
        `);
        const workspaceIds = workspaceWithoutUser.map((workspace) => `'${workspace.id}'`).join(',');
        ...
        if (workspaceIds && workspaceIds.length > 0) {
            ...
            `);
            const chatflowIds = chatflows.map((chatflow) => `'${chatflow.id}'`).join(',');
            ...
            `);
            const documentStoreIds = documentStores.map((documentStore) => `'${documentStore.id}'`).join(',');
            ...
            `);
            const evaluationIds = evaluations.map((evaluation) => `'${evaluation.id}'`).join(',');
    ...
    async populateTable(queryRunner) {
        // insert generalRole
        const generalRole = [
            {
                name: 'owner',
        ...
        const ssoConfig = organizations[0].sso_config ? JSON.parse(await (0, encryption_util_1.decrypt)(organizations[0].sso_config)).providers : [];
        /*-------------------------------------
        --------------- user -----------------
        --------------------------------------*/
        ...
        for (let config of ssoConfig) {
            ...
            const status = config.configEnabled === true ? login_method_entity_1.LoginMethodStatus.ENABLE : login_method_entity_1.LoginMethodStatus.DISABLE;
            const allUndefined = Object.values(newConfigFormat).every((value) => value === undefined);
            if (allUndefined && status === login_method_entity_1.LoginMethodStatus.DISABLE)
                continue;
            const encryptData = await (0, encryption_util_1.encrypt)(JSON.stringify(newConfigFormat));
            await queryRunner.query(`
        ...
        for (let role of workspaceRole) {
            role.permissions = JSON.stringify(role.permissions.split(',').filter((permission) => permission.trim() !== ''));
        ...
        for (let user of users) {
            const roleId = user.id === adminUserId
                ? roles.find((role) => role.name === role_entity_1.GeneralRole.OWNER).id
                : roles.find((role) => role.name === role_entity_1.GeneralRole.MEMBER).id;
            await queryRunner.query(`
        ...
        for (let workspaceUser of workspaceUsers) {
            switch (workspaceUser.role) {
                case 'org_admin':
                    workspaceUser.role = roles.find((role) => role.name === role_entity_1.GeneralRole.OWNER).id;
                    break;
                case 'pw':
                    workspaceUser.role = roles.find((role) => role.name === role_entity_1.GeneralRole.PERSONAL_WORKSPACE).id;
                    break;
                default:
                    workspaceUser.role = roles.find((role) => role.name === workspaceUser.role).id;
                    break;
            }
            const user = users.find((user) => user.id === workspaceUser.userId);
            const workspace = workspaces.find((workspace) => workspace.id === workspaceUser.workspaceId);
            if (workspaceUser.workspaceId === user.activeWorkspaceId && user.lastLogin && user.status !== user_entity_1.UserStatus.INVITED) {
                ...
                await queryRunner.query(`
                        ...
                        values ('${workspaceUser.workspaceId}','${workspaceUser.userId}','${workspaceUser.role}','${workspace_user_entity_1.WorkspaceUserStatus.ACTIVE}','${lastLogin}','${adminUserId}','${adminUserId}');
                    `);
            }
            else if (workspace.name === workspace_entity_1.WorkspaceName.DEFAULT_PERSONAL_WORKSPACE && user.status === user_entity_1.UserStatus.INVITED) {
                ...
                await queryRunner.query(`
                        ...
                        values ('${workspaceUser.workspaceId}','${workspaceUser.userId}','${workspaceUser.role}','${workspace_user_entity_1.WorkspaceUserStatus.INVITED}','${adminUserId}','${adminUserId}');
                    `);
            }
        }
        await this.deleteWorkspaceWithoutUser(queryRunner);
    }
    async deleteTempTable(queryRunner) {
        await queryRunner.query(`
    ...
    async up(queryRunner) {
        ...
        await (0, _1729130948686_LinkWorkspaceId_1.linkWorkspaceId)(queryRunner, false);
        await (0, _1743758056188_FixOpenSourceAssistantTable_1.fixOpenSourceAssistantTable)(queryRunner);
    }
    async down() { }
}
exports.RefactorEnterpriseDatabase1737076223692 = RefactorEnterpriseDatabase1737076223692;
