...
class AccountController {
    ...
    async cancelPreviousCloudSubscrption(req, res, next) {
        try {
            const { email } = req.body;
            if (!email) {
                return res.status(http_status_codes_1.StatusCodes.BAD_REQUEST).json({ message: 'Email is required' });
            }
            const headers = {
                'Content-Type': 'application/json',
                Accept: 'application/json'
            };
            const response = await axios_1.default.post(`${process.env.ENGINE_URL}/cancel-subscription`, { email }, { headers });
