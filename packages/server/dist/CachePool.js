...
class CachePool {
     ...
     * @param {any} value
     */
    async addSSOTokenCache(ssoToken, value) {
        if (process.env.MODE === Interface_1.MODE.QUEUE) {
            if (this.redisClient) {
    ...
    async getSSOTokenCache(ssoToken) {
        if (process.env.MODE === Interface_1.MODE.QUEUE) {
            if (this.redisClient) {
    ...
    async deleteSSOTokenCache(ssoToken) {
        if (process.env.MODE === Interface_1.MODE.QUEUE) {
            if (this.redisClient) {
     ...
     * @param {any} value
     */
    async addMCPCache(cacheKey, value) {
